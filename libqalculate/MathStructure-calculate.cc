...
int MathStructure::merge_multiplication(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct, bool reversed, bool do_append) {
	...
	if(mstruct.isFunction() && eo.protected_function != mstruct.function()) {
		if(((mstruct.function()->id() == FUNCTION_ID_ABS && mstruct.size() == 1 && mstruct[0].isFunction() && mstruct[0].function()->id() == FUNCTION_ID_SIGNUM && mstruct[0].size() == 2) || (mstruct.function()->id() == FUNCTION_ID_SIGNUM && mstruct.size() == 2 && mstruct[0].isFunction() && mstruct[0].function()->id() == FUNCTION_ID_ABS && mstruct[0].size() == 1)) && (equals(mstruct[0][0]) || (isFunction() && o_function->id() == FUNCTION_ID_ABS && SIZE == 1 && CHILD(0) == mstruct[0][0]) || (isPower() && CHILD(0) == mstruct[0][0]) || (isPower() && CHILD(0).isFunction() && CHILD(0).function()->id() == FUNCTION_ID_ABS && CHILD(0).size() == 1 && CHILD(0)[0] == mstruct[0][0]))) {
			...
			MERGE_APPROX_AND_PREC(mstruct)
			return 1;
		} else if(mstruct.function()->id() == FUNCTION_ID_LAMBERT_W && mstruct.size() == 2 && m_type == STRUCT_POWER && CHILD(1) == mstruct && CHILD(0).isVariable() && CHILD(0).variable()->id() == VARIABLE_ID_E && (mstruct[1].isZero() || mstruct[0].representsNonZero())) {
			SET_CHILD_MAP(1)
			SET_CHILD_MAP(0)
			MERGE_APPROX_AND_PREC(mstruct)
			return 1;
...
int MathStructure::merge_logical_and(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct, bool) {
	...
	if(representsScalar() && mstruct.representsScalar()) {
		if(equals(mstruct, true, true)) {
			MERGE_APPROX_AND_PREC(mstruct)
			return 2;
		}
		if(mstruct.representsNonZero()) {
			if(mparent) mparent->mergePrecision(mstruct);
			else {MERGE_APPROX_AND_PREC(mstruct)}
			return 2;
		}
		if(mstruct.isZero()) {
			if(isZero()) return 2;
...
bool contains_approx_num_without_interval(const MathStructure &m) {
	if(m.isNumber()) return m.number().isApproximate() && !m.number().isInterval();
	if(m.isVariable() && m.variable()->isKnown()) return contains_approx_num_without_interval(((KnownVariable*) m.variable())->get());
	for(size_t i = 0; i < m.size(); i++) {
		if(contains_approx_num_without_interval(m[i])) return true;
	}
	return false;
}
...
bool MathStructure::calculatesub(const EvaluationOptions &eo, const EvaluationOptions &feo, bool recursive, MathStructure *mparent, size_t index_this) {
	...
	switch(m_type) {
		...
		case STRUCT_LOGICAL_AND: {
			if(recursive) {
				bool comp = false;
				bool scalar = representsScalar();
				for(size_t i = 0; i < SIZE; i++) {
					CHILD(i).calculatesub(eo, feo, true, this, i);
					CHILD_UPDATED(i)
					if(scalar && CHILD(i).isZero()) {
						clear(true);
						b = true;
						break;
					}
				...
				if(comp) {
					for(size_t i = 0; i < SIZE; i++) {
						if(CHILD(i).isLogicalNot() || ((scalar || CHILD(i).representsScalar()) && !CHILD(i).representsBoolean())) {
			...
			if(SIZE == 1) {
				if(CHILD(0).representsBoolean() || (mparent && !mparent->isMultiplication() && mparent->representsBoolean()) || !CHILD(0).representsScalar()) {
					setToChild(1, false, mparent, index_this + 1);
		...
		case STRUCT_LOGICAL_OR: {
			if(recursive) {
				...
				for(size_t i = 0; i < SIZE; i++) {
					CHILD(i).calculatesub(eo, feo, true, this, i);
					CHILD_UPDATED(i)
					if(scalar && CHILD(i).representsNonZero()) {
						set(1, 1, 0, true);
						b = true;
						break;
					}
		...
		case STRUCT_LOGICAL_XOR: {
			if(recursive) {
				...
				if(comp) {
					for(size_t i = 0; i < SIZE; i++) {
						if(CHILD(i).isLogicalNot() || (CHILD(i).representsScalar() && !CHILD(i).representsBoolean())) {
							if(CHILD(i).isLogicalNot()) {
								CHILD(i).setType(STRUCT_COMPARISON);
								CHILD(i).setComparisonType(COMPARISON_EQUALS);
								CHILD(i).addChild(m_zero);
		...
		case STRUCT_LOGICAL_NOT: {
			...
			if(CHILD(0).representsNonZero()) {
				...
				calculatesub(eo, feo, false);
				b = true;
			} else if(CHILD(0).isLogicalNot() && CHILD(0).representsScalar()) {
				// !(!a)=a
				setToChild(1);
				setToChild(1);
				...
				b = true;
			} else if(!CHILD(0).representsBoolean() && !CHILD(0).isUnknown() && CHILD(0).representsScalar()) {
				m_type = STRUCT_COMPARISON;
				ct_comp = COMPARISON_EQUALS;
				APPEND(m_zero)
			}
		...
		case STRUCT_COMPARISON: {
			...
			if(((CHILD(0).isNumber() || (CHILD(0).isVariable() && !CHILD(0).variable()->isKnown() && ((UnknownVariable*) CHILD(0).variable())->interval().isNumber())) && (CHILD(1).isNumber() || ((CHILD(1).isVariable() && !CHILD(1).variable()->isKnown() && ((UnknownVariable*) CHILD(1).variable())->interval().isNumber())))) || (CHILD(0).isDateTime() && CHILD(1).isDateTime())) {
				ComparisonResult cr;
				bool keep_approx = false;
				if(CHILD(0).isDateTime()) {
					cr = CHILD(1).compare(CHILD(0));
				} else {
					const Number *nr0 = NULL, *nr1 = NULL;
					if(CHILD(0).isNumber()) nr0 = &CHILD(0).number();
					else nr0 = &((UnknownVariable*) CHILD(0).variable())->interval().number();
					if(CHILD(1).isNumber()) nr1 = &CHILD(1).number();
					else nr1 = &((UnknownVariable*) CHILD(1).variable())->interval().number();
					if(!keep_approx && nr0->isApproximate() && (nr0->precision() >= 0 || !nr0->isInterval())) keep_approx = true;
					if(!keep_approx && nr1->isApproximate() && (nr1->precision() >= 0 || !nr1->isInterval())) keep_approx = true;
					cr = nr1->compareApproximately(*nr0);
					if(cr == COMPARISON_RESULT_EQUAL && !keep_approx) keep_approx = !nr1->equals(*nr0);
				}
			...
			if(CHILD(1).isNumber() && CHILD(0).isVariable() && !CHILD(0).variable()->isKnown()) {
				...
				if(ass && ass->min()) {
					...
					switch(ct_comp) {
						case COMPARISON_EQUALS: {
							if((b_inc && CHILD(1).number() < *ass->min()) || (!b_inc && CHILD(1).number() <= *ass->min())) {SET_FALSE_TPA; b = true;}
							break;
						}
						case COMPARISON_NOT_EQUALS: {
							if((b_inc && CHILD(1).number() < *ass->min()) || (!b_inc && CHILD(1).number() <= *ass->min())) {SET_TRUE_TPA; b = true;}
							break;
						}
						case COMPARISON_LESS: {
							if(CHILD(1).number() <= *ass->min()) {SET_FALSE_TPA; b = true;}
						...
						case COMPARISON_EQUALS_LESS: {
							...
							else if((b_inc && CHILD(1).number() < *ass->min()) || (!b_inc && CHILD(1).number() <= *ass->min())) {SET_FALSE_TPA; b = true;}
							break;
						}
						case COMPARISON_EQUALS_GREATER: {
							if(CHILD(1).number() <= *ass->min()) {SET_TRUE_TPA; b = true;}
							break;
						}
					}
				}
				if(ass && ass->max() && isComparison()) {
					...
					switch(ct_comp) {
						case COMPARISON_EQUALS: {
							if((b_inc && CHILD(1).number() > *ass->max()) || (!b_inc && CHILD(1).number() >= *ass->max())) {SET_FALSE_TPA; b = true;}
							break;
						}
						case COMPARISON_NOT_EQUALS: {
							if((b_inc && CHILD(1).number() > *ass->max()) || (!b_inc && CHILD(1).number() >= *ass->max())) {SET_TRUE_TPA; b = true;}
							break;
						}
						case COMPARISON_LESS: {
							if((b_inc && CHILD(1).number() > *ass->max()) || (!b_inc && CHILD(1).number() >= *ass->max())) {SET_TRUE_TPA; b = true;}
						...
						case COMPARISON_GREATER: {
							if(CHILD(1).number() >= *ass->max()) {SET_FALSE_TPA; b = true;}
							break;
						}
						case COMPARISON_EQUALS_LESS: {
							if(CHILD(1).number() >= *ass->max()) {SET_TRUE_TPA; b = true;}
							break;
						}
						case COMPARISON_EQUALS_GREATER: {
							if(b_inc && CHILD(1).number() == *ass->max()) {ct_comp = COMPARISON_EQUALS; b = true;}
							else if((b_inc && CHILD(1).number() > *ass->max()) || (!b_inc && CHILD(1).number() >= *ass->max())) {SET_FALSE_TPA; b = true;}
							break;
						}
					}
				}
