...
int find_variable_n(const MathStructure &m) {
	if(m.isVariable()) {
		const string &name = m.variable()->referenceName();
		if(name == "n") {
			return 1;
		} else if(name.length() >= 2 && name[0] == 'n' && name[1] > '0' && name[1] <= '9') {
			for(size_t i = 2; i < name.length(); i++) {
				if(name[i] < '0' || name[i] > '9') return 0;
			}
			return s2i(name.substr(1, name.length() - 1));
		}
	}
	int n = 0;
	for(size_t i = 0; i < m.size(); i++) {
		int n_i = find_variable_n(m[i]);
		if(n_i > n) n = n_i;
	}
	return n;
}

Variable *find_next_variable_n(const MathStructure &m, int n, int *index_p = NULL) {
	if(m.isVariable() && !m.variable()->isKnown()) {
		const string &name = m.variable()->referenceName();
		if(n <= 1 && name == "n") {
			if(index_p) *index_p = 1;
			return m.variable();
		} else if(name.length() >= 2 && name[0] == 'n' && name[1] > '0' && name[1] <= '9') {
			for(size_t i = 2; i < name.length(); i++) {
				if(name[i] < '0' || name[i] > '9') return 0;
			}
			int i = s2i(name.substr(1, name.length() - 1));
			if(i >= n) {
				if(index_p) *index_p = i;
				return m.variable();
			}
		}
	}
	...
	for(size_t i = 0; i < m.size(); i++) {
		int n_i = 0;
		Variable *v_i = find_next_variable_n(m[i], n, &n_i);
		if(v_i && (index == 0 || ((n >= 0 && n_i < index) || (n < 0 && n_i > index)))) {
			v = v_i;
			index = n_i;
			if(index == n) break;
		}
...
MathStructure *get_variable_n(const MathStructure &m) {
	int n = find_variable_n(m);
	if(n == 0) return new MathStructure(CALCULATOR->getVariableById(VARIABLE_ID_N));
	UnknownVariable *var = new UnknownVariable("", string("n") + i2s(n + 1));
	var->setAssumptions(new Assumptions());
	var->assumptions()->setType(ASSUMPTION_TYPE_INTEGER);
	MathStructure *mvar = new MathStructure(var);
	var->destroy();
	return mvar;
}

bool fix_n_multiple(MathStructure &mstruct, const EvaluationOptions &eo, const EvaluationOptions &feo, const MathStructure &x_var, int index = -1, Variable *v_n = NULL) {
	if(!v_n) {
		bool b = false;
		if(index == 0) index = 1;
		while(true) {
			int n = 0;
			v_n = find_next_variable_n(mstruct, index, &n);
			if(!v_n) return b;
			if(fix_n_multiple(mstruct, eo, feo, x_var, index, v_n)) b = true;
			if(index < 0) break;
			index = n + 1;
		}
		return b;
	...
	if(mstruct.isComparison()) {
		if(mstruct.comparisonType() == COMPARISON_EQUALS && x_var.isVariable() && !x_var.variable()->isKnown() && (!((UnknownVariable*) x_var.variable())->interval().isUndefined() || (((UnknownVariable*) x_var.variable())->assumptions() && ((((UnknownVariable*) x_var.variable())->assumptions()->min() || x_var.representsNonNegative()) && (((UnknownVariable*) x_var.variable())->assumptions()->max() || x_var.representsNonPositive())))) && mstruct[1].contains(v_n)) {
			...
			if(CALCULATOR->usesIntervalArithmetic()) {
				...
				if(mtest.isolate_x(eo2, feo2, v_n)) {
					if(CALCULATOR->endTemporaryStopMessages() == 0) {
						if(mtest.isZero()) {
							SET_FALSE_TPA_M(mstruct)
							MERGE_TPA_M(mstruct, mtest)
							b_ret = true;
						} else if(mtest.isComparison() && mtest.comparisonType() == COMPARISON_EQUALS && mtest[0].isVariable() && mtest[0].variable() == v_n && mtest[1].isNumber()) {
							if(mtest[1].number().isInteger()) {
								mstruct.calculateReplace(v_n, mtest[1], eo);
								b_ret = true;
								...
								if(mtest[1].number().getCentralInteger(nr_int, &b_multiple)) {
									mstruct.calculateReplace(v_n, nr_int, eo);
									b_ret = true;
								} else if(!b_multiple) {
									SET_FALSE_TPA_M(mstruct)
									MERGE_TPA_M(mstruct, mtest)
		...
		for(size_t i = 0; i < mstruct.size(); i++) {
			if(fix_n_multiple(mstruct[i], eo, feo, x_var, index, v_n)) {
...
#define SECOND_DEGREE_TEST_COMPLEX	if(CHILD(0).representsReal(true)) {\
			...
			if((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) && CHILD(0).size() > 1) {
				...
				if(mvar) {
					...
					if(mvar && get_multiplier(CHILD(0), *mvar, m_c, i_px) && m_c.representsNonZero() && m_c.contains(x_var) == 0 && m_b.representsNonZero()) {
						...
						if(marg && mvar->representsNonComplex() && m_b.representsPositive()) {
							...
							marg->transformById(FUNCTION_ID_LAMBERT_W);
							marg->addChild_nocopy(get_variable_n(*this));
							if(marg->calculateFunctions(eo)) marg->calculatesub(eo2, eo, true);
							marg->calculateNegate(eo2);
							if(CHILD(1).isZero()) {
								marg->calculateDivide(mlogb, eo2);
				...
				if(CHILD(0).containsFunctionId(FUNCTION_ID_LOG)) {
					...
					if(mvar && mln) {
						...
						size_t ln_i = 0;
						if(CHILD(0).size() == 2 && &CHILD(0)[0] == mvar) ln_i = 1;
						if(CHILD(0).size() == 2 && ((mvar->isPower() && (*mvar)[1] == (*mln)[0] && (((*mvar)[0].isVariable() && (*mvar)[0].variable() == CALCULATOR->getVariableById(VARIABLE_ID_E)) || ((*mvar)[0].isNumber() && (*mvar)[0].number().compare(((KnownVariable*) CALCULATOR->getVariableById(VARIABLE_ID_E))->get().number()) == COMPARISON_RESULT_EQUAL_LIMITS)) && CHILD(0)[ln_i].isMultiplication() && CHILD(0)[ln_i].size() == 2 && CHILD(0)[ln_i][0].isMinusOne()) || (mvar->isMultiplication() && mvar->size() == 2 && (*mvar)[0].isMinusOne() && (*mvar)[1].isPower() && (*mvar)[1][1] == (*mln)[0] && (((*mvar)[1][0].isVariable() && (*mvar)[1][0].variable() == CALCULATOR->getVariableById(VARIABLE_ID_E)) || ((*mvar)[1][0].isNumber() && (*mvar)[1][0].number().compare(((KnownVariable*) CALCULATOR->getVariableById(VARIABLE_ID_E))->get().number()) == COMPARISON_RESULT_EQUAL_LIMITS)) && &CHILD(0)[ln_i] == mln))) {
							// e^x = ln(x)
							...
							CHILD(0).setToChild(1, true);
							CHILD(1).set(m_minus_one, true);
							CHILD(1).transformById(FUNCTION_ID_LAMBERT_W);
							MathStructure *malt = new MathStructure(*this);
							CHILD(1).addChild(m_zero);
							(*malt)[1].addChild(m_minus_one);
							if(CHILD(1).calculateFunctions(eo)) CHILD(1).calculatesub(eo2, eo, true);
							if((*malt)[1].calculateFunctions(eo)) (*malt)[1].calculatesub(eo2, eo, true);
							CHILD(1).calculateNegate(eo2);
			...
			if(CHILD(0).size() >= 2 && CHILD(1).isZero() && (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
				// a*x^b+c*d^x=0 => -b*lambertw(-root(-c/a, b)*ln(d)/b)/ln(d) || -b*lambertw(root(-c/a, b)*ln(d)/b)/ln(d) || -b*lambertw(-root(-c/a, b)*ln(d)/b, -1)/ln(d) || -b*lambertw(root(-c/a, b)*ln(d)/b, -1)/ln(d)
				...
				for(size_t i = 0; i < CHILD(0).size(); i++) {
					if(CHILD(0)[i].isPower()) {
						...
						if(b1) {
							...
							if((m2 && !CHILD(0)[i].equals(*m2)) || (!m2 && !m1 && !CHILD(0)[i][1].representsReal()) || (!m2 && m1 && !CHILD(0)[i][1].equals((*m1)[0]))) {m1 = NULL; break;}
							else if(!m2) m2 = &CHILD(0)[i];
						}
					} else if(CHILD(0)[i].isMultiplication() && CHILD(0)[i].size() >= 2) {
						...
						for(size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
							if(CHILD(0)[i][i2].contains(x_var)) {
								...
								if(b1) {
									...
									if((m2 && !CHILD(0)[i][i2].equals(*m2)) || (!m2 && !m1 && !CHILD(0)[i][i2][1].representsReal()) || (!m2 && m1 && !CHILD(0)[i][i2][1].equals((*m1)[0]))) {m1 = NULL; m2 = NULL; break;}
									else if(!m2) m2 = &CHILD(0)[i][i2];
								}
							} else if(!CHILD(0)[i][i2].representsReal()) {
								m1 = NULL; m2 = NULL; break;
							}
						}
						if(!m1 && !m2) break;
				...
				if(m1 && m2) {
					...
					if(CHILD(0).size() == 2 && CHILD(0)[0].isPower() && CHILD(0)[1].isPower()) {
						mroot = new MathStructure(mlog);
					} else {
						MathStructure *m_a = new MathStructure(), *m_c = new MathStructure();
						for(size_t i = 0; i < CHILD(0).size(); i++) {
							if(CHILD(0)[i].isPower()) {
								MathStructure *m_i = m_c;
								if(CHILD(0)[i][0].contains(x_var)) m_i = m_a;
								if(m_i->isZero()) m_i->set(1, 1, 0);
								else m_i->add(m_one, true);
							} else if(CHILD(0)[i].isMultiplication()) {
								for(size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
									if(CHILD(0)[i][i2].contains(x_var)) {
										MathStructure *m_i = m_c;
										if(CHILD(0)[i][i2][0].contains(x_var)) m_i = m_a;
										if(m_i->isZero()) {
											m_i->set(CHILD(0)[i]);
											m_i->delChild(i2 + 1, true);
										} else {
											m_i->add(CHILD(0)[i], true);
											m_i->last().delChild(i2 + 1, true);
										}
										break;
									}
								}
					...
					CHILD(0).set(mvar, true);
					ERASE(1)
					APPEND_POINTER(mroot)
					MathStructure *malt = new MathStructure(mvar);
					malt->transform_nocopy(STRUCT_COMPARISON, mroot2);
					malt->setComparisonType(ct_comp_bak);
					MathStructure *malt2 = new MathStructure(mvar);
					malt2->transform_nocopy(STRUCT_COMPARISON, mroot3);
					malt2->setComparisonType(ct_comp_bak);
					MathStructure *malt3 = new MathStructure(mvar);
					malt3->transform_nocopy(STRUCT_COMPARISON, mroot4);
					malt3->setComparisonType(ct_comp_bak);
					malt->isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
					malt2->isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
					malt3->isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
			...
			if(m && m->function()->id() == FUNCTION_ID_ABS) {
				...
				if((ct_comp_bak == COMPARISON_EQUALS && malt->isLogicalOr()) || (ct_comp_bak == COMPARISON_NOT_EQUALS && malt->isLogicalAnd())) {
					for(size_t i = 0; i < malt->size(); i++) {
						(*malt)[i].add_nocopy(i != malt->size() - 1 ? new MathStructure(*mcheck) : mcheck, ct_comp_bak == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR : OPERATION_LOGICAL_AND, true);
		...
		case STRUCT_MULTIPLICATION: {
			...
			if(CHILD(0).size() == 2 && (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
				size_t abs_i = 0, i = 0;
				if(CHILD(0)[0].isFunction() && CHILD(0)[0].function()->id() == FUNCTION_ID_ABS) i = 1;
				else if(CHILD(0)[1].isFunction() && CHILD(0)[1].function()->id() == FUNCTION_ID_ABS) abs_i = 1;
				if(abs_i != i && CHILD(0)[abs_i].size() == 1) {
					if(CHILD(0)[abs_i][0] == CHILD(0)[i]) {
						// x*abs(x)
						if(CHILD(1).representsNonNegative(true)) {
							CHILD(1).raise_nocopy(new MathStructure(1, 2, 0));
							CHILD(1).calculateRaiseExponent(eo2);
							CHILD(0).setToChild(i + 1, true);
							isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
							return true;
							...
							mabs->calculateFunctions(eo);
							mabs->raise_nocopy(new MathStructure(-1, 2, 0));
							mabs->calculateRaiseExponent(eo2);
							CHILD(1).multiply_nocopy(mabs);
							CHILD(1).calculateMultiplyLast(eo2);
							CHILD(0).setToChild(i + 1, true);
							isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
							return true;
						}
					} else if(CHILD(0)[i].isPower() && CHILD(0)[abs_i][0] == CHILD(0)[i][0] && CHILD(0)[i][1] == nr_two) {
						// x^2*abs(x)
						if(CHILD(1).representsComplex()) {
							...
							if(CHILD(1).isNumber() && !CHILD(1).number().hasRealPart() && CHILD(1).number().imaginaryPartIsNonZero()) {
								bool neg = CHILD(1).number().imaginaryPartIsNegative();
								if(neg) CHILD(1).calculateMultiply(nr_one_i, eo2);
								else CHILD(1).calculateMultiply(nr_minus_i, eo2);
								CHILD(1).raise_nocopy(new MathStructure(1, 3, 0));
								CHILD(1).calculateRaiseExponent(eo2);
								CHILD(1).add_nocopy(new MathStructure(CHILD(1)));
								if(neg) CHILD(1).last().calculateMultiply(nr_minus_i, eo2);
								else CHILD(1).last().calculateMultiply(nr_one_i, eo2);
								CHILD(1).calculateAddLast(eo2);
								CHILD(1).multiply_nocopy(new MathStructure(2, 1, 0));
								CHILD(1).last().raise_nocopy(new MathStructure(-1, 2, 0));
								CHILD(1).last().calculateRaiseExponent(eo2);
								CHILD(1).calculateMultiplyLast(eo2);
								CHILD(0).setToChild(i + 1, true);
								CHILD(0).setToChild(1, true);
								MathStructure *malt = new MathStructure(*this);
								(*malt)[1].calculateNegate(eo);
								ComparisonType ct_comp_bak = ct_comp;
								isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
								malt->isolate_x_sub(eo, eo2, x_var, NULL, depth + 1);
								add_nocopy(malt, ct_comp_bak == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_AND : OPERATION_LOGICAL_OR, true);
						...
						} else if(CHILD(1).representsNonNegative(true)) {
							CHILD(1).raise_nocopy(new MathStructure(1, 3, 0));
							CHILD(1).calculateRaiseExponent(eo2);
							CHILD(0).setToChild(i + 1, true);
							CHILD(0).setToChild(1, true);
							...
							CHILD(1).raise_nocopy(new MathStructure(1, 3, 0));
							CHILD(1).calculateRaiseExponent(eo2);
							CHILD(1).calculateMultiply(nr_one_i, eo2);
							CHILD(0).setToChild(i + 1, true);
							CHILD(0).setToChild(1, true);
							MathStructure *malt = new MathStructure(*this);
							(*malt)[1].calculateNegate(eo);
		...
		case STRUCT_POWER: {
			if(CHILD(0)[0].contains(x_var)) {
				if(CHILD(0)[1].contains(x_var)) {
					if((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS) && !CHILD(1).isZero()) {
						...
						if(CHILD(0)[0].representsNonComplex() && CHILD(1).compare(m_zero) == COMPARISON_RESULT_LESS) {
							...
							marg->last().multiply_nocopy(get_variable_n(*this), true);
							marg->evalSort(true);
							MathStructure *mreq2 = NULL;
				...
				if((ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
					if(!CHILD(0)[0].representsNonZero()) {
						if(!CHILD(1).representsNonZero()) {
							...
							m0->transform(ct_comp == COMPARISON_EQUALS ? COMPARISON_GREATER : COMPARISON_LESS, m_zero);
						...
						if(!CHILD(0)[0].isZero()) {
							...
							mtest->add_nocopy(mtest_b, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND : OPERATION_LOGICAL_OR);
							mtest->calculatesub(eo2, eo, false);
						}
					}
					if(!CHILD(1).representsNonZero()) {
						...
						MathStructure *mtest_b = new MathStructure(CHILD(1));
						mtest_b->transform(ct_comp == COMPARISON_EQUALS ? COMPARISON_NOT_EQUALS : COMPARISON_EQUALS, m_zero);
						mtest_b->isolate_x_sub(eo, eo2, x_var, morig, depth + 1);
						if(mtest) {
							mtest->add_nocopy(mtest_b, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND : OPERATION_LOGICAL_OR, true);
							mtest->calculatesub(eo2, eo, false);
						} else {
							mtest = mtest_b;
						}
					...
					ComparisonResult cr1 = CHILD(0)[0].compare(m_one);
					if(cr1 == COMPARISON_RESULT_EQUAL) {
						...
						if(cr2 == COMPARISON_RESULT_EQUAL) {
							...
							m1->add_nocopy(m1b, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND : OPERATION_LOGICAL_OR);
							m1->calculatesub(eo2, eo, false);
						}
					}
					if(COMPARISON_MIGHT_BE_EQUAL(cr1)) {
						MathStructure *mn1 = new MathStructure();
						...
						if(mtest) {
							mtest->add_nocopy(mn1, ct_comp == COMPARISON_EQUALS ? OPERATION_LOGICAL_AND : OPERATION_LOGICAL_OR, true);
							mtest->calculatesub(eo2, eo, false);
						} else {
							mtest = mn1;
						}
				...
				if(m0) add_nocopy(m0, ct_comp_bak == COMPARISON_EQUALS ? OPERATION_LOGICAL_OR : OPERATION_LOGICAL_AND);
				if(m1) add_nocopy(m1, ct_comp_bak == COMPARISON_EQUALS ? OPERATION_LOGICAL_OR : OPERATION_LOGICAL_AND, m0 != NULL);
				if(m0 || m1) calculatesub(eo2, eo, false);
				fix_n_multiple(*this, eo2, eo, x_var);
				return true;
			}
			break;
		...
		case STRUCT_FUNCTION: {
			if(CHILD(0).function()->id() == FUNCTION_ID_ROOT && CHILD(0).size() == 2) {
				...
				CHILD(1)[1].multiply_nocopy(get_variable_n(*this));
				...
				if(CHILD(0).representsNonComplex()) {
					...
					(*malt)[1] *= nr_minus_i;
					CHILD(1) += CALCULATOR->getVariableById(VARIABLE_ID_PI);
					MathStructure *m_n = get_variable_n(*this);
					MathStructure *m_n2 = new MathStructure(*m_n);
					CHILD(1).add_nocopy(m_n); CHILD(1).last() *= CALCULATOR->getVariableById(VARIABLE_ID_PI); CHILD(1).last() *= nr_two;
					(*malt)[1].add_nocopy(m_n2); (*malt)[1].last() *= CALCULATOR->getVariableById(VARIABLE_ID_PI); (*malt)[1].last() *= nr_two;
					CHILD(1) *= nr_one_i;
					(*malt)[1] *= nr_one_i;
					CHILD(1).calculatesub(eo2, eo, true);
					(*malt)[1].calculatesub(eo2, eo, true);
				...
				if(CHILD(0).representsNonComplex()) {
					...
					CHILD(1) *= nr_minus_i;
					CHILD(1).add_nocopy(get_variable_n(*this)); CHILD(1).last() *= CALCULATOR->getVariableById(VARIABLE_ID_PI);
					CHILD(1) *= nr_one_i;
					CHILD(1).calculatesub(eo2, eo, true);
					CHILDREN_UPDATED;
					isolate_x_sub(eo, eo2, x_var, morig, depth + 1);
...
bool MathStructure::isolate_x(const EvaluationOptions &eo, const EvaluationOptions &feo, const MathStructure &x_varp, bool check_result, size_t depth) {
	...
	fix_n_multiple(*this, eo, feo, x_var, 1);
	if(check_result && b) {
