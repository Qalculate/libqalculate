		...
		while ((u->subtype() == SUBTYPE_BASE_UNIT || (u->subtype() == SUBTYPE_ALIAS_UNIT && ((AliasUnit *)u)->firstBaseUnit()->subtype() != SUBTYPE_COMPOSITE_UNIT && ((AliasUnit *)u)->firstBaseExponent() == 1 && (((AliasUnit *)u)->mixWithBase() != 0 || muc == MIXED_UNITS_CONVERSION_FORCE_ALL || muc == MIXED_UNITS_CONVERSION_FORCE_INTEGER))) && !nr.isInteger() && nr.isNonZero())
		{
			...
			if (u->subtype() == SUBTYPE_ALIAS_UNIT && (muc == MIXED_UNITS_CONVERSION_FORCE_ALL || (((AliasUnit *)u)->expression().find_first_not_of(NUMBERS) == string::npos)))
			{
				...
				if (mstruct_nr.isNumber() && m_exp.isOne())
				{
					...
					{
						if (((AliasUnit *)u)->mixWithBase() > 0 && muc > MIXED_UNITS_CONVERSION_DEFAULT)
							muc = MIXED_UNITS_CONVERSION_DEFAULT;
						u = ((AliasUnit *)u)->firstBaseUnit();
...
MathStructure Calculator::convert(double value, Unit *from_unit, Unit *to_unit, const EvaluationOptions &eo)
{
	...
	if (eo.approximation == APPROXIMATION_EXACT)
	{
		...
		mstruct.convert(to_unit, true, nullptr, false, eo2);
	}
	else
	{
		mstruct.convert(to_unit, true, nullptr, false, eo);
	}
...
{
	if (top && mp.isUnit_exp())
		return false;
	for (size_t i = ip; i < mp.size(); i++)
	{
		if (mp[i].isInverse() || mp[i].isDivision() || mp[i].isMultiplication())
		{
			if (cu_contains_nonunit(mp[i], top && i == 0 && !mp[i].isInverse() && mp[i][0].isNumber() ? 1 : 0, false))
				return true;
		}
		else if (mp[i].isPower())
		{
			if (cu_contains_nonunit(mp[i][0], 0, false))
				return true;
		}
		else if (!mp[i].isUnit())
		{
...
void cu_fix_formatted(MathStructure &m)
{
	if (m.isInverse())
	{
		...
		else if (m[0].isMultiplication())
		{
			...
			{
				if (m[i].isPower() && m[i][1].isNumber())
					m[i][1].number().negate();
				else
					m[i].raise(m_minus_one);
			}
		}
		else
		{
	...
	else if (m.isDivision())
	{
		...
		else if (m[1].isMultiplication())
		{
			...
			{
				if (m[1][i].isPower() && m[1][i][1].isNumber())
					m[1][i][1].number().negate();
				else
					m[1][i].raise(m_minus_one);
			}
		}
		else
		{
	...
	if (b)
	{
		...
		if (mr.isMultiplication())
		{
			for (size_t i = 0; i < mr.size(); i++)
			{
				...
				if (mr[i].isUnit())
					ui = mr[i].unit();
				else if (mr[i].isPower() && mr[i][0].isUnit())
					ui = mr[i][0].unit();
				if (ui)
				{
					if (!u)
					...
					n++;
					if (!b && ui->subtype() != SUBTYPE_BASE_UNIT && !ui->isSIUnit())
						b = true;
				}
			}
		}
		...
		else if (b)
		{
			...
			if (eo.auto_post_conversion == POST_CONVERSION_OPTIMAL || eo.auto_post_conversion == POST_CONVERSION_NONE)
			{
				if (n == 1 && no_exp && u->referenceName() == "oF")
				{
					...
					if (u)
						mr.set(CALCULATOR->convert(mr, u, eo, true, false, false));
				}
			...
			{
				eo.approximation = APPROXIMATION_TRY_EXACT;
				if (eo.auto_post_conversion == POST_CONVERSION_BASE)
					mr.set(CALCULATOR->convertToBaseUnits(mr, eo));
				else
					mr.set(CALCULATOR->convertToOptimalUnit(mr, eo, true));
...
void fix_to_struct(MathStructure &m)
{
	if (m.isPower() && m[0].isUnit())
	{
		...
		if (m[0].prefix() == nullptr && m[0].unit()->defaultPrefix() != 0)
		{
	...
	else if (m.isUnit())
	{
		...
		if (m.prefix() == nullptr && m.unit()->defaultPrefix() != 0)
		{
	...
	else
	{
		for (size_t i = 0; i < m.size();)
		{
			if (m[i].isUnit())
			{
				...
				if (m[i].prefix() == nullptr && m[i].unit()->defaultPrefix() != 0)
				{
			...
			else if (m[i].isPower() && m[i][0].isUnit())
			{
				...
				if (m[i][0].prefix() == nullptr && m[i][0].unit()->defaultPrefix() != 0)
				{
...
void fix_to_struct2(MathStructure &m)
{
	...
	else
	{
		for (size_t i = 0; i < m.size();)
		{
			if (m[i].isUnit())
			{
				m[i].setPrefix(nullptr);
				i++;
			}
			else if (m[i].isPower() && m[i][0].isUnit())
			{
				m[i][0].setPrefix(nullptr);
				i++;
			}
			else
			{
...
{
	if (m.isFunction() && (m.function()->id() == FUNCTION_ID_ATAN || m.function()->id() == FUNCTION_ID_ACOS || m.function()->id() == FUNCTION_ID_ASIN || m.function()->id() == FUNCTION_ID_ARG || m.function()->id() == FUNCTION_ID_ATAN2 || m.function()->id() == FUNCTION_ID_RADIANS_TO_DEFAULT_ANGLE_UNIT))
		return true;
	if (m.isFunction() && m.function()->subtype() == SUBTYPE_USER_FUNCTION)
	{
		UserFunction *f = (UserFunction *)m.function();
		if (f->formula().find("arctan") != string::npos || f->formula().find("arccos") != string::npos || f->formula().find("arcsin") != string::npos || f->formula().find("atan(") != string::npos || f->formula().find("acos(") != string::npos || f->formula().find("asin(") != string::npos)
			return true;
	}
	if (m.isVariable() && m.variable()->isKnown())
		return contains_angle_returning_function(((KnownVariable *)m.variable())->get());
	for (size_t i = 0; i < m.size(); i++)
	{
...
MathStructure get_units_for_parsed_expression(const MathStructure *parsed_struct, Unit *to_unit, const EvaluationOptions &eo, const MathStructure *mstruct, string parsed_text)
{
	...
	if (cu && cu->countUnits() == 0)
		return m_zero;
	if ((mstruct && mstruct->containsType(STRUCT_UNIT, true)) || (!mstruct && parsed_struct->containsType(STRUCT_UNIT, false, true, true)))
		return m_zero;
	int exp1, exp2;
	...
	if (!b_ratio || b_angle)
	{
		if (mstruct)
		{
			...
			if (!(ct == 0 || (b_angle && (ct < 0 || (parsed_struct && !contains_angle_ratio(*parsed_struct))))))
				return m_zero;
		}
		EvaluationOptions eo2 = eo;
		if (eo.approximation == APPROXIMATION_EXACT)
...
{
	if (n <= 0 || (eo2.auto_post_conversion != POST_CONVERSION_OPTIMAL && eo2.auto_post_conversion != POST_CONVERSION_OPTIMAL_SI) || CALCULATOR->aborted())
		return;
...
{
	if (m.isUnit())
		return m.unit() != CALCULATOR->getRadUnit();
	for (size_t i = 0; i < m.size(); i++)
	{
		if (contains_non_angle_unit_cc(m[i]))
			return true;
	}
	return false;
}
...
MathStructure Calculator::convert(const MathStructure &mstruct, Unit *to_unit, const EvaluationOptions &eo, bool always_convert, bool convert_to_mixed_units, bool transform_orig, MathStructure *parsed_struct)
{
			...
			if (!munit.isZero())
			{
				...
				if (mstruct_new.isComparison() || (mstruct_new.isLogicalAnd() && mstruct_new.size() > 0 && mstruct_new[0].isComparison()) || (mstruct_new.isLogicalOr() && mstruct_new.size() > 0 && (mstruct_new[0].isComparison() || (mstruct_new[0].isLogicalAnd() && mstruct_new[0].size() > 0 && mstruct_new[0][0].isComparison()))))
				{
					if (mstruct_new.isLogicalOr())
					{
						...
						{
							if (mstruct_new[i].isComparison())
								mstruct_new[i][1] *= munit;
							else if (mstruct_new[i].isLogicalAnd() && mstruct_new[i].size() > 0 && mstruct_new[i][0].isComparison())
								mstruct_new[i][0][1] *= munit;
						}
					}
					else
					{
						if (mstruct_new.isLogicalAnd())
							mstruct_new[0][1] *= munit;
						else
							mstruct_new[1] *= munit;
					}
				...
				return convert(mstruct_new, to_unit, eo, always_convert, convert_to_mixed_units, false, nullptr);
			}
		}
		if (!b_angle)
			return mstruct;
	...
	if (to_unit->hasNonlinearRelationTo(to_unit->baseUnit()) && to_unit->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT)
	{
		if (mstruct_new.isComparison() || (mstruct_new.isLogicalAnd() && mstruct_new.size() > 0 && mstruct_new[0].isComparison()) || (mstruct_new.isLogicalOr() && mstruct_new.size() > 0 && (mstruct_new[0].isComparison() || (mstruct_new[0].isLogicalAnd() && mstruct_new[0].size() > 0 && mstruct_new[0][0].isComparison()))))
		{
			if (mstruct_new.isLogicalOr())
			{
				...
				{
					if (mstruct_new[i].isComparison())
						mstruct_new[i][1].set(convert(mstruct_new[i][1], to_unit, eo, always_convert, convert_to_mixed_units, transform_orig));
					else if (mstruct_new[i].isLogicalAnd() && mstruct_new[i].size() > 0 && mstruct_new[i][0].isComparison())
						mstruct_new[i][0][1].set(convert(mstruct_new[i][0][1], to_unit, eo, always_convert, convert_to_mixed_units, transform_orig));
				}
			}
			else
			{
				if (mstruct_new.isLogicalAnd())
					mstruct_new[0][1].set(convert(mstruct_new[0][1], to_unit, eo, always_convert, convert_to_mixed_units, transform_orig));
				else
					mstruct_new[1].set(convert(mstruct_new[1], to_unit, eo, always_convert, convert_to_mixed_units, transform_orig));
			}
			mstruct_new.childrenUpdated(true);
			return mstruct_new;
		}
		...
		mstruct_new.eval(eo);
		mstruct_new.multiply(MathStructure(to_unit, eo.keep_prefixes ? decimal_null_prefix : nullptr));
		EvaluationOptions eo2 = eo;
		eo2.sync_units = false;
	...
	if (mstruct_new.size() > 0 && !mstruct_new.isPower() && !mstruct_new.isUnit() && !mstruct_new.isMultiplication())
	{
		if (mstruct_new.size() > 0)
		{
			for (size_t i = 0; i < mstruct_new.size(); i++)
			{
				...
				{
					mstruct_new[i] = convert(mstruct_new[i], to_unit, eo, false, convert_to_mixed_units, false, nullptr);
	...
	else
	{
		...
		if (transform_orig)
		{
			...
			{
				const MathStructure *mstruct_u = nullptr;
		...
		if (mstruct_new.convert(to_unit, true, nullptr, false, eo2, eo.keep_prefixes ? decimal_null_prefix : nullptr))
		{
			b = true;
		}
		else if (b_ratio)
		{
			...
			if (cu->get(1, &exp)->baseUnit()->referenceName() == "m")
			{
				...
				if (u)
				{
					mstruct_new.convert(u, true, nullptr, false, eo2, nullptr);
		...
		else if (to_unit->baseUnit() == getRadUnit() && mstruct_new.contains(to_unit, true) < 1)
		{
			...
			b = mstruct_new.convert(to_unit, true, nullptr, false, eo2, eo.keep_prefixes ? decimal_null_prefix : nullptr) || always_convert;
		}
		...
		if (b)
		{
			...
			if (to_unit != priv->u_celsius && to_unit != priv->u_fahrenheit)
			{
				...
				mstruct_new.divide_nocopy(new MathStructure(to_unit, nullptr));
				mstruct_new.eval(eo2);
				...
				if (cu2)
					b_pos = true;
				else if (to_unit->baseUnit()->subtype() == SUBTYPE_COMPOSITE_UNIT)
				...
				if (n > 0)
				{
					...
					{
						mtest.inverse();
						mtest.divide_nocopy(new MathStructure(to_unit, nullptr));
						mtest.eval(eo2);
					...
					if (n > 0 && (!cu || (cu->countUnits() == 1 && (cu->get(1, &exp) && exp == 1))))
					{
						...
						while (exp > -10 && n > 0)
						{
							mtest.multiply_nocopy(new MathStructure(to_unit, nullptr));
							mtest.eval(eo2);
							long int ntest = count_unit_powers(mtest);
				...
				if (!b_ratio && !b_angle && contains_angle_unit_cc(mstruct_new) == 3)
					mstruct_new.convert(getRadUnit(), false, nullptr, true, eo2);
				if (cu)
				{
					MathStructure *mstruct_cu = new MathStructure(cu->generateMathStructure(false, eo.keep_prefixes));
					Prefix *p = nullptr;
					size_t i = 1;
					Unit *u = cu->get(i, nullptr, &p);
					while (u)
					{
						size_t i2 = i + 1;
						if (b_eval)
				...
				{
					mstruct_new.multiply_nocopy(new MathStructure(to_unit, eo.keep_prefixes ? decimal_null_prefix : nullptr));
					if (exp != 1)
...
MathStructure Calculator::convertToBaseUnits(const MathStructure &mstruct, const EvaluationOptions &eo)
{
	...
	size_t n_messages = messages.size();
	mstruct_new.convertToBaseUnits(true, nullptr, true, eo);
				...
				if (units[i]->isActive() && units[i]->subtype() == SUBTYPE_COMPOSITE_UNIT && !units[i]->isHidden() && units[i]->isSIUnit())
				{
					...
					if (cu2->countUnits() <= cu->countUnits())
					{
						for (size_t i2 = 1; i2 <= cu2->countUnits(); i2++)
						{
							...
							{
								MathStructure *cu_mstruct = nullptr;
								...
								else
								{
									...
									if (cu_mstruct->convertToBaseUnits(true, nullptr, true))
									{
										beginTemporaryStopMessages();
										cu_mstruct->eval();
										endTemporaryStopMessages();
...
MathStructure Calculator::convertToOptimalUnit(const MathStructure &mstruct, const EvaluationOptions &eo, bool convert_to_si_units)
{
	...
	case STRUCT_POWER:
	{
		if (mstruct.isUnit() || (mstruct.base()->isUnit() && mstruct.exponent()->isNumber() && mstruct.exponent()->number().isRational() && !mstruct.exponent()->number().isZero()))
		{
			...
			if (mstruct.isUnit())
				old_points = 1;
			else if (mstruct.exponent()->isInteger())
				old_points = mstruct.exponent()->number().intValue(&overflow);
			else
				old_points = mstruct.exponent()->number().numerator().intValue(&overflow) + mstruct.exponent()->number().denominator().intValue() * (mstruct.exponent()->number().isNegative() ? -1 : 1);
			if (overflow)
				return mstruct;
			bool is_si_units = u->isSIUnit();
			if (old_points == 1 && (is_si_units || !convert_to_si_units) && (!eo.local_currency_conversion || !u->isCurrency() || u == getLocalCurrency()))
				return mstruct;
			...
			{
				mstruct_new.convertToBaseUnits(true, nullptr, true, eo2, true);
				if (mstruct_new.equals(mstruct, true, true))
				{
				...
				for (size_t i = 1; i <= mstruct_new.countChildren(); i++)
				{
					...
					else if (mstruct_new.getChild(i)->isPower() && mstruct_new.getChild(i)->base()->isUnit() && mstruct_new.getChild(i)->exponent()->isNumber() && mstruct_new.getChild(i)->exponent()->number().isRational())
					{
						...
						if (mstruct_new.getChild(i)->exponent()->isInteger())
							...
							points = mstruct_new.getChild(i)->exponent()->number().numerator().intValue() + mstruct_new.getChild(i)->exponent()->number().denominator().intValue() * (mstruct_new.getChild(i)->exponent()->number().isNegative() ? -1 : 1);
						if (new_is_si_units && !mstruct_new.getChild(i)->base()->unit()->isSIUnit())
							new_is_si_units = false;
			...
			else if (mstruct_new.isPower() && mstruct_new.base()->isUnit() && mstruct_new.exponent()->isNumber() && mstruct_new.exponent()->number().isRational())
			{
				...
				if (mstruct_new.exponent()->isInteger())
					...
					points = mstruct_new.exponent()->number().numerator().intValue() + mstruct_new.exponent()->number().denominator().intValue() * (mstruct_new.exponent()->number().isNegative() ? -1 : 1);
	...
	{
		if ((mstruct.isFunction() && mstruct.function()->id() == FUNCTION_ID_STRIP_UNITS) || !mstruct.containsType(STRUCT_UNIT, true))
			return mstruct;
		MathStructure mstruct_new(mstruct);
		bool b = false;
	...
	case STRUCT_MULTIPLICATION:
	{
		...
		for (size_t i = 1; i <= mstruct_old.countChildren(); i++)
		{
			...
			else if (mstruct_old.getChild(i)->isPower() && mstruct_old.getChild(i)->base()->isUnit() && mstruct_old.getChild(i)->exponent()->isNumber() && mstruct_old.getChild(i)->exponent()->number().isRational())
			{
				...
				if (mstruct_old.getChild(i)->exponent()->number().isInteger())
					...
					points = mstruct_old.getChild(i)->exponent()->number().numerator().intValue() + mstruct_old.getChild(i)->exponent()->number().denominator().intValue() * (mstruct_old.getChild(i)->exponent()->number().isNegative() ? -1 : 1);
				;
		...
		{
			if (!mstruct_new.containsInterval(true, true, false, 1, true) && !aborted())
				mstruct_new = convertToOptimalUnit(mstruct_new, eo, convert_to_si_units);
		}
		else
		{
...
{
	return convert(mstruct_to_convert, str2, eo, to_struct, false, nullptr);
}
...
MathStructure Calculator::convert(const MathStructure &mstruct_to_convert, string str2, const EvaluationOptions &eo, MathStructure *to_struct, bool transform_orig, MathStructure *parsed_struct)
{
	...
	if (v)
	{
		...
		if (!u)
		{
			...
			else if (v->referenceName() == "compton_wavelength_2pi")
				u = CALCULATOR->getActiveUnit("l_N");
		}
		if (u)
			v = nullptr;
	}
