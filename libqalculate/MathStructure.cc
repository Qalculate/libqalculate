...
bool MathStructure::isMatrix() const
{
	...
	for (size_t i = 0; i < SIZE; i++)
	{
		if (!CHILD(i).isVector() || (i > 0 && CHILD(i).size() != CHILD(i - 1).size()))
			return false;
	}
	return true;
}
...
bool MathStructure::representsNumber(bool allow_units) const
{
	...
	case STRUCT_POWER:
	{
		if (!CHILD(0).representsNonZero(allow_units) && !CHILD(1).representsPositive(allow_units))
			return false;
		[[fallthrough]];
	}
...
bool MathStructure::representsPositive(bool allow_units) const
{
	...
	case STRUCT_POWER:
	{
		return (CHILD(0).representsPositive(allow_units) && CHILD(1).representsReal(false)) || (CHILD(0).representsNonZero(allow_units) && CHILD(0).representsReal(allow_units) && CHILD(1).representsEven(false) && CHILD(1).representsInteger(false));
	}
	default:
	{
		return false;
...
bool MathStructure::representsNonNegative(bool allow_units) const
{
	...
	case STRUCT_POWER:
	{
		return (CHILD(0).isZero() && CHILD(1).representsNonNegative(false)) || (CHILD(0).representsNonNegative(allow_units) && CHILD(1).representsReal(false)) || (CHILD(0).representsReal(allow_units) && CHILD(1).representsEven(false) && CHILD(1).representsInteger(false));
	}
	default:
	{
		return false;
...
bool MathStructure::representsReal(bool allow_units) const
{
	...
	case STRUCT_POWER:
	{
		return (CHILD(0).representsPositive(allow_units) && CHILD(1).representsReal(false)) || (CHILD(0).representsReal(allow_units) && CHILD(1).representsInteger(false) && (CHILD(1).representsPositive(false) || CHILD(0).representsNonZero(allow_units)));
	}
	default:
	{
		return false;
...
bool MathStructure::representsFinite(bool allow_units) const
{
	...
	case STRUCT_VARIABLE:
	{
		if (o_variable->isKnown())
			return ((KnownVariable *)o_variable)->get().representsFinite(allow_units);
		return o_variable->representsReal(allow_units);
	}
	case STRUCT_SYMBOLIC:
	{
	...
	{
		if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
			return CHILD(0).representsFinite(true);
...
bool MathStructure::representsNonComplex(bool allow_units) const
{
	...
	{
		if (o_variable->isKnown())
			return ((KnownVariable *)o_variable)->get().representsNonComplex(allow_units);
	...
	case STRUCT_POWER:
	{
		return (CHILD(0).representsPositive(allow_units) && CHILD(1).representsReal(false)) || (CHILD(0).representsReal(allow_units) && CHILD(1).representsInteger(false));
	}
	default:
	{
		return false;
...
bool MathStructure::representsUndefined(bool include_childs, bool include_infinite, bool be_strict) const
{
	...
	{
		if (o_function->id() == FUNCTION_ID_STRIP_UNITS && SIZE == 1)
			return CHILD(0).representsUndefined(include_childs, include_infinite, be_strict);
	...
	case STRUCT_MULTIPLICATION:
	{
		if (SIZE > 1 && CHILD(0).isZero() && CHILD(1).isInfinity())
			return true;
		[[fallthrough]];
	}
	default:
	{
		if (include_childs)
		{
			for (size_t i = 0; i < SIZE; i++)
			{
				if (CHILD(i).representsUndefined(include_childs, include_infinite, be_strict))
					return true;
			}
		}
		return false;
...
void MathStructure::setChild_nocopy(MathStructure *o, size_t index, bool merge_precision)
{
	if (index > 0 && index <= SIZE)
	{
		...
		if (merge_precision)
		{
			...
			if (o_prev->precision() >= 0 && (o->precision() < 0 || o_prev->precision() < o->precision()))
...
{
	if (m.isUnit() && m.unit()->baseUnit() == CALCULATOR->getRadUnit()->baseUnit() && m.unit()->baseExponent() == 1)
		return 1;
	if (m.isVariable() && m.variable()->isKnown())
		return contains_angle_unit(((KnownVariable *)m.variable())->get(), po, check_functions);
	if (m.isFunction())
	{
...
{
	if (mstruct.isUnit() && mstruct.prefix() == nullptr && m_type == STRUCT_UNIT)
	...
	if (structural_only)
	{
		for (size_t i = 0; i < SIZE; i++)
		{
			if (CHILD(i).contains(mstruct, structural_only, check_variables, check_functions, loose_equals))
				return 1;
		}
...
{
	if (mstruct.isUnit() && mstruct.prefix() == nullptr && m_type == STRUCT_UNIT && mstruct.unit() == o_unit)
		return 1;
...
int MathStructure::containsFunction(MathFunction *f, bool structural_only, bool check_variables, bool check_functions) const
{
	...
	if (structural_only)
	{
		for (size_t i = 0; i < SIZE; i++)
		{
			if (CHILD(i).containsFunction(f, structural_only, check_variables, check_functions))
				return 1;
		}
		if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown())
		{
...
int MathStructure::containsFunctionId(int id, bool structural_only, bool check_variables, bool check_functions) const
{
	...
	if (structural_only)
	{
		for (size_t i = 0; i < SIZE; i++)
		{
			if (CHILD(i).containsFunctionId(id, structural_only, check_variables, check_functions))
				return 1;
		}
		if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown())
		{
...
int contains_interval_var(const MathStructure &m, bool structural_only, bool check_variables, bool check_functions, int ignore_high_precision_interval, bool include_interval_function)
{
	if (m.type() == STRUCT_NUMBER)
	{
		if (m.number().isInterval(false))
		{
			...
			{
				if (m.number().precision(true) > (ignore_high_precision_interval < 0 ? (ignore_high_precision_interval == -1 ? PRECISION + 29 : PRECISION - ignore_high_precision_interval) : PRECISION + (10 * ignore_high_precision_interval)))
					return 0;
			}
			return 1;
		}
		else if (CALCULATOR->usesIntervalArithmetic() && m.number().precision() >= 0)
		{
			if (ignore_high_precision_interval != 0)
			{
				if (m.number().precision() > (ignore_high_precision_interval < 0 ? (ignore_high_precision_interval == -1 ? PRECISION + 29 : PRECISION - ignore_high_precision_interval) : PRECISION + (10 * ignore_high_precision_interval)))
					return 0;
			}
			return 1;
		}
	}
	if (m.type() == STRUCT_FUNCTION && (m.function()->id() == FUNCTION_ID_INTERVAL || m.function()->id() == FUNCTION_ID_UNCERTAINTY))
		return include_interval_function;
	if (structural_only)
	{
		for (size_t i = 0; i < m.size(); i++)
		{
			if (contains_interval_var(m[i], structural_only, check_variables, check_functions, ignore_high_precision_interval, include_interval_function))
				return 1;
		}
...
int MathStructure::containsInterval(bool structural_only, bool check_variables, bool check_functions, int ignore_high_precision_interval, bool include_interval_function) const
{
	if (m_type == STRUCT_NUMBER && o_number.isInterval(false))
	{
		if (ignore_high_precision_interval != 0)
		{
			if (o_number.precision(true) > (ignore_high_precision_interval < 0 ? (ignore_high_precision_interval == -1 ? PRECISION + 29 : PRECISION - ignore_high_precision_interval) : PRECISION + (10 * ignore_high_precision_interval)))
				return 0;
		}
		return 1;
	}
	if (m_type == STRUCT_FUNCTION && (o_function->id() == FUNCTION_ID_INTERVAL || o_function->id() == FUNCTION_ID_UNCERTAINTY))
		return include_interval_function;
	if (structural_only)
	{
		for (size_t i = 0; i < SIZE; i++)
		{
			if (CHILD(i).containsInterval(structural_only, check_variables, check_functions, ignore_high_precision_interval, include_interval_function))
				return 1;
		}
		if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown())
		{
...
int MathStructure::containsInfinity(bool structural_only, bool check_variables, bool check_functions) const
{
	...
	if (structural_only)
	{
		for (size_t i = 0; i < SIZE; i++)
		{
			if (CHILD(i).containsInfinity(structural_only, check_variables, check_functions))
				return 1;
		}
		if (m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown())
		{
...
int MathStructure::containsRepresentativeOf(const MathStructure &mstruct, bool check_variables, bool check_functions) const
{
	...
	if (m_type == STRUCT_VARIABLE && check_variables)
	{
		if (o_variable->isKnown())
			...
			return ((UnknownVariable *)o_variable)->interval().containsRepresentativeOf(mstruct, check_variables, check_functions);
	}
	else if (m_type == STRUCT_FUNCTION && check_functions)
	{
		if (function_value)
	...
	else
	{
		...
		if (check_variables && m_type == STRUCT_VARIABLE)
		{
			if (o_variable->isKnown())
				...
				return ((UnknownVariable *)o_variable)->interval().containsType(mtype, false, check_variables, check_functions);
		...
		else if (check_functions && m_type == STRUCT_FUNCTION)
		{
			...
			{
				if (o_function->id() == FUNCTION_ID_COLON || o_function->id() == FUNCTION_ID_GENERATE_VECTOR || o_function->id() == FUNCTION_ID_VERTCAT || o_function->id() == FUNCTION_ID_HORZCAT || o_function->id() == FUNCTION_ID_MATRIX || o_function->id() == FUNCTION_ID_VECTOR)
					return 1;
			}
			else if (mtype == STRUCT_UNIT)
			{
				if (o_function->id() == FUNCTION_ID_STRIP_UNITS)
					return 0;
				if (o_function->subtype() == SUBTYPE_USER_FUNCTION || o_function->subtype() == SUBTYPE_DATA_SET || o_function->id() == FUNCTION_ID_REGISTER || o_function->id() == FUNCTION_ID_STACK || o_function->id() == FUNCTION_ID_LOAD)
					return -1;
...
{
	if (m_type == STRUCT_DIVISION || m_type == STRUCT_INVERSE || (m_type == STRUCT_POWER && CHILD(1).hasNegativeSign()))
		return true;
	for (size_t i = 0; i < SIZE; i++)
	{
	...
	if (mfrom.size() > 0 && mfrom.type() == m_type && SIZE > mfrom.size() && (mfrom.isAddition() || mfrom.isMultiplication() || mfrom.isLogicalAnd() || mfrom.isLogicalOr()))
	{
		...
		if (b)
		{
			...
			if (SIZE == 1)
				...
				replace(mfrom, mto, once_only, exclude_function_arguments, replace_in_variables);
			if (mfrom.isAddition())
				add(mto);
...
bool MathStructure::removeType(StructureType mtype)
{
	...
	else
	{
		if (m_type == STRUCT_FUNCTION)
		{
			if (mtype != STRUCT_UNIT || (o_function->id() != FUNCTION_ID_SQRT && o_function->id() != FUNCTION_ID_ROOT && o_function->id() != FUNCTION_ID_CBRT))
				return b;
		}
		for (size_t i = 0; i < SIZE; i++)
		{
