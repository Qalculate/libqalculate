					...
					if(b && divs > 0) {
						if(SIZE - divs == 0) {
							if(mstruct.size() - divs == 0) {
								calculateMultiply(MathStructure(2, 1, 0), eo);
		...
		case STRUCT_FUNCTION: {
			if(((o_function == CALCULATOR->f_abs && SIZE == 1 && CHILD(0).isFunction() && CHILD(0).function() == CALCULATOR->f_signum && CHILD(0).size() == 1) || (o_function == CALCULATOR->f_signum && SIZE == 1 && CHILD(0).isFunction() && CHILD(0).function() == CALCULATOR->f_abs && CHILD(0).size() == 1)) && (CHILD(0)[0] == mstruct || (mstruct.isFunction() && mstruct.function() == CALCULATOR->f_abs && mstruct.size() == 1 && CHILD(0)[0] == mstruct[0]) || (mstruct.isPower() && mstruct[0] == CHILD(0)[0]) || (mstruct.isPower() && mstruct[0].isFunction() && mstruct[0].function() == CALCULATOR->f_abs && mstruct[0].size() == 1 && CHILD(0)[0] == mstruct[0][0]))) {
				...
				if(mstruct.isFunction() && mstruct.function() == CALCULATOR->f_abs && mstruct.size() == 1 && mstruct[0] == CHILD(0) && CHILD(0).representsReal(true)) {
					...
					SET_CHILD_MAP(0)
					MERGE_APPROX_AND_PREC(mstruct)
					calculateRaise(MathStructure(2, 1, 0), eo);
					return 1;
...
bool try_polynomial_division(MathStructure &mstruct, const EvaluationOptions &eo) {
	if(!mstruct.isAddition() || !eo.reduce_divisions || !eo.expand) return false;
	MathStructure divs;
	...
	for(size_t i = 0; i < divs.size(); ) {
		if(divs[i].number().isZero() || (!eo.assume_denominators_nonzero && !divs[i].representsNonZero(true)) || divs[i].representsZero(true)) divs.delChild(i + 1);
		else i++;
	}
	...
	for(size_t i = 0; i < divs.size(); i++) {
		if(nums[i].size() > 1) {
			...
			if(nums[i].isRationalPolynomial()) {
				...
				if(polynomial_long_division(nums[i], divs[i], m_zero, mquo, mrem, eo, false) && !mquo.isZero() && mrem != nums[i]) {
...
bool MathStructure::calculatesub(const EvaluationOptions &eo, const EvaluationOptions &feo, bool recursive, MathStructure *mparent, size_t index_this) {	
	...
	switch(m_type) {
		...
		case STRUCT_ADDITION: {
			...
			if(try_polynomial_division(*this, eo)) {
				calculatesub(eo, feo, recursive, mparent, index_this);
				b = true;
			}
...
int newton_raphson(const MathStructure &mstruct, MathStructure &x_value, const MathStructure &x_var, const EvaluationOptions &eo) {
	...
	for(int i = 0; i < 100 + PRECISION + ideg * 2; i++) {
		...
		mtest.eval(eo);
		if(!mtest.isNumber() || !mguess.number().divide(mtest.number())) return -1;
...
bool MathStructure::isolate_x_sub(const EvaluationOptions &eo, EvaluationOptions &eo2, const MathStructure &x_var, MathStructure *morig) {
	...
	switch(CHILD(0).type()) {
		case STRUCT_ADDITION: {
			...
			if(CHILD(0).size() == 2 && (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
				...
				if(sqpow && nopow && mstruct_a.representsNonZero(true)) {
					...
					b2.calculateRaise(MathStructure(2, 1, 0), eo2);
			...
			if(eo2.expand && CHILD(0).size() <= 10 && CHILD(0).size() > 1) {
				int i_root = 0;
				size_t root_index = 0;
				size_t root_index2 = 0;
				for(size_t i = 0; i < CHILD(0).size(); i++) {
					if(CHILD(0)[i].isPower()) {
						if(!i_root && CHILD(0)[i][1].isNumber() && CHILD(0)[i][1].number().numeratorIsOne() && CHILD(0)[i][1].number().denominator().isLessThan(10)) {
						...
						if(!i_root && VALID_ROOT(CHILD(0)[i]) && CHILD(0)[i][1].number().isLessThan(10)) {
						...
						for(size_t i2 = 0; i2 < CHILD(0)[i].size(); i2++) {
							if(CHILD(0)[i][i2].isPower()) {
								if(CHILD(0)[i][i2][1].isNumber() && CHILD(0)[i][i2][1].number().isInteger()) {
									...
									b_xvar = true;
								} else if(!i_root && !b_xvar && CHILD(0)[i][i2][1].isNumber() && CHILD(0)[i][i2][1].number().numeratorIsOne() && CHILD(0)[i][i2][1].number().denominator().isLessThan(10)) {
								...
								if(!i_root && !b_xvar && VALID_ROOT(CHILD(0)[i][i2]) && CHILD(0)[i][i2][1].number().isLessThan(10)) {
				...
				if(i_root) {
					...
					if(mtest[0].isMultiplication()) {
						if(mtest[0].size() == 2) {
							mtest[1].calculateDivide(mtest[0][root_index2 == 0 ? 1 : 0], eo2);
						} else if(mtest[0].size() > 2) {
							MathStructure mdiv(mtest[1]);
							mdiv.delChild(root_index2 + 1);
							mtest[1].calculateDivide(mdiv, eo2);
						}
						mtest[0].setToChild(root_index2 + 1);
					}
					if(mtest.isolate_x_sub(eo, eo2, x_var, morig)) {
						set(mtest);
						isolate_x(eo2, eo, x_var);
						return true;
					}
					break;
				}
		...
		case STRUCT_MULTIPLICATION: {
			...
			// x/(x+a)=b => x=b(x+a)
			if(CHILD(1).isNumber() && (ct_comp == COMPARISON_EQUALS || ct_comp == COMPARISON_NOT_EQUALS)) {
				for(size_t i = 0; i < CHILD(0).size(); i++) {
					if(CHILD(0)[i].isPower() && CHILD(0)[i][1].isNumber() && CHILD(0)[i][1].number().isNegative() && CHILD(0)[i][1].number().isReal()) {
						...
						mcheck.add(m_zero, ct_comp == COMPARISON_NOT_EQUALS ? OPERATION_EQUALS : OPERATION_NOT_EQUALS);
						mcheck.isolate_x_sub(eo, eo2, x_var);
						CHILD(0).delChild(i + 1);
						...
						CHILD(1).clear();
						isolate_x_sub(eo, eo2, x_var, morig);
						add(mcheck, cmp_type == COMPARISON_NOT_EQUALS ? OPERATION_LOGICAL_OR : OPERATION_LOGICAL_AND, true);
						calculatesub(eo2, eo, false);
						return true;
					}
				}
				...
				if(ct_comp == COMPARISON_EQUALS && eo2.approximation != APPROXIMATION_EXACT && CHILD(1).isNumber() && CHILD(1).number().isPositive() && CHILD(0)[1] == x_var && CHILD(0)[0] == x_var && CHILD(0)[0].representsReal(true)) {
					...
					if(b_neg && CHILD(1).isZero()) {
						...
						if(ct_comp == COMPARISON_EQUALS_GREATER || ct_comp == COMPARISON_GREATER) {
							ct_comp = COMPARISON_LESS;
						} else if(ct_comp == COMPARISON_EQUALS_LESS || ct_comp == COMPARISON_LESS) {
							ct_comp = COMPARISON_GREATER;
						}
						CHILD(1).clear(true);
						CHILD(0).setToChild(1);
