...
bool warn_about_assumed_not_value(const MathStructure &mstruct, const MathStructure &mvalue, const EvaluationOptions &eo) {
	...
	if(mnonzero.isComparison() && mnonzero[0].isVariable() && !mnonzero[0].variable()->isKnown() && !((UnknownVariable*) mnonzero[0].variable())->interval().isUndefined()) {
		if(((UnknownVariable*) mnonzero[0].variable())->interval().containsInterval(true)) {
...
ComparisonResult MathStructure::compare(const MathStructure &o) const {
	...
	if(isMultiplication() && SIZE > 1 && CHILD(0).isNumber()) {
		if(o.isZero()) {
			...
			if(b) return CHILD(0).number().compare(o.number());
...
int MathStructure::merge_multiplication(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct, bool reversed, bool do_append) {
	...
	if(m_type == STRUCT_NUMBER && o_number.isInfinite()) {
		if(o_number.isMinusInfinity(true)) {
		...
		} else if(o_number.isPlusInfinity(true)) {
			if(mstruct.representsPositive(false)) {
				MERGE_APPROX_AND_PREC(mstruct)
				return 2;
			} else if(mstruct.representsNegative(false)) {
		...
		if(mstruct.number().isMinusInfinity(true)) {
		...
		} else if(mstruct.number().isPlusInfinity(true)) {
			if(representsPositive(false)) {
				clear(true);
				o_number.setPlusInfinity();
				MERGE_APPROX_AND_PREC(mstruct)
...
bool limit_combine_divisions(MathStructure &mstruct) {
	if(mstruct.isAddition()) {
		bool b = false;
		// 5/y + x/y + z = (5 + x)/y + z
		for(size_t i = 0; i < mstruct.size() && !b; i++) {
			if(mstruct[i].isMultiplication()) {
				for(size_t i2 = 0; i2 < mstruct[i].size() && !b; i2++) {
					if(mstruct[i][i2].isPower() && mstruct[i][i2][1].representsNegative()) {
						b = true;
					}
				}
			} else if(mstruct[i].isPower() && mstruct[i][1].representsNegative()) {
				b = true;
			}
		}
		if(!b) return false;
		...
		for(size_t i = 0; i < mstruct_units.size(); i++) {
			if(mstruct_units[i].isMultiplication()) {
				for(size_t i2 = 0; i2 < mstruct_units[i].size();) {
					if(!mstruct_units[i][i2].isPower() || !mstruct_units[i][i2][1].representsNegative()) {
						mstruct_units[i].delChild(i2 + 1);
					} else {
						i2++;
					}
				...
				for(size_t i2 = 0; i2 < mstruct_new[i].size();) {
					if(mstruct_new[i][i2].isPower() && mstruct_new[i][i2][1].representsNegative()) {
						mstruct_new[i].delChild(i2 + 1);
					} else {
						i2++;
					}
			...
			} else if(mstruct_new[i].isPower() && mstruct_new[i][1].representsNegative()) {
				mstruct_new[i].set(1, 1, 0);
			} else {
				mstruct_units[i].setUndefined();
			}
...
bool limit_factor_xvar(MathStructure &mstruct, const MathStructure &x_var) {
	if(mstruct.isAddition()) {
		bool b = false;
		MathStructure mstruct_units(mstruct);
		MathStructure mstruct_new(mstruct);
...
bool calculate_limit_sub(MathStructure &mstruct, const MathStructure &x_var, const MathStructure &nr_limit, const EvaluationOptions &eo, int approach_direction, Number *polydeg = NULL, int lhop_depth = 0, bool keep_inf_x = false, bool reduce_addition = true) {
	...
	switch(mstruct.type()) {
		case STRUCT_MULTIPLICATION: {
			...
			for(size_t i = 0; i < mstruct.size(); i++) {
				if(mstruct[i].isPower() && mstruct[i][1].representsNegative()) {
					if(mden.isOne()) {
						mden = mstruct[i];
						mden[1].calculateNegate(eo);
					} else {
						mden.multiply(mstruct[i], true);
						mden.last()[1].calculateNegate(eo);
					}
				}
			}
			if(!mden.isOne()) {
				...
				if(!mnum.isOne()) {
					...
					if(nr_limit.isInfinite(false)) {
						...
						if(mnum.contains(x_var, true) && mden.contains(x_var, true)) {
							...
							if(cmp == 0) {
								...
								if(!mstruct.contains(x_var)) return true;
							}
							MathStructure mnum_b, mden_b;
							if(keep_inf_x) {mnum_b = mnum; mden_b = mden;}
							...
							if(mnum.isInfinite(false) && mden.isInfinite(false)) {
								if(cmp > 0) {
									if(keep_inf_x) {
										...
										mden_b.calculateInverse(eo);
										mstruct.multiply(mden_b, true);
										break;
									}
						...
						} else if(mnum.contains(x_var, true)) {
							calculate_limit_sub(mnum, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, false);
						} else if(mden.contains(x_var, true)) {
							calculate_limit_sub(mden, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, false);
						}
					} else {
						calculate_limit_sub(mnum, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, false);
					...
					if(lhop_depth < 5 && ((mnum.isInfinite(false) && mden.isInfinite(false)) || (mnum.isZero() && mden.isZero()))) {
						...
						if(mnum_bak.differentiate(x_var, eo) && !mnum_bak.containsFunction(CALCULATOR->f_diff, true) && mden_bak.differentiate(x_var, eo) && !mden_bak.containsFunction(CALCULATOR->f_diff, true)) {
							mnum_bak /= mden_bak;
							mnum_bak.eval(eo);
							calculate_limit_sub(mnum_bak, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth + 1);
							mstruct.set(mnum_bak, true);
							break;
						}
					}
					if(mden.isNumber() && !mden.number().isNonZero() && !mnum.isZero()) {
					...
					mstruct.set(mnum, true);
					mstruct.calculateDivide(mden, eo);
					break;
				}
			}
			...
			bool b_inf = false;
			for(size_t i = 0; i < mstruct.size(); i++) {
				calculate_limit_sub(mstruct[i], x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, keep_inf_x);
				if(mstruct[i].isZero()) {
			...
			if(lhop_depth < 5 && b_inf && !mzero.isOne()) {
				mzero.inverse();
				MathStructure minf(1, 1, 0);
				for(size_t i = 0; i < mstruct.size(); i++) {
					if(!mstruct[i].isZero()) {
						if(minf.isOne()) minf = mbak[i];
						else minf.multiply(mbak[i], true);
					}
				}
				//L'Hôpital's rule
				if(minf.differentiate(x_var, eo) && !minf.containsFunction(CALCULATOR->f_diff, true) && mzero.differentiate(x_var, eo) && !mzero.containsFunction(CALCULATOR->f_diff, true)) {
					minf /= mzero;
					minf.eval(eo);
					calculate_limit_sub(minf, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth + 1);
					mstruct.set(minf, true);
					break;
				}
			}
		...
		case STRUCT_ADDITION: {
			...
			if(nr_limit.isInfinite(false)) {
				...
				if(mstruct.contains(x_var, true) && (!keep_inf_x || reduce_addition)) {
					limit_factor_xvar(mstruct, x_var);
					for(size_t i = 0; i < mstruct.size(); i++) {
		...
		case STRUCT_POWER: {
			...
			if(nr_limit.isInfinite(false)) {
				...
				if(mstruct[1].isInfinite(false) && mstruct[0].isOne() && mbak[1].contains(x_var, true) && mbak[0].isAddition() && mbak[0].contains(x_var, true)) {
					bool b_one = false;
					for(size_t i = 0; i < mbak[0].size(); i++) {
						if(mbak[0][i].isOne()) {
							b_one = true;
							break;
						}
					...
					if(b_one) {
						...
						MathStructure *mmul = NULL;
						MathStructure mbase(mbak[0]);
						limit_combine_divisions(mbase);
						if(mbase.isAddition() && mbase.size() == 2) {
							if(mbase[0].isOne()) mpow = &mbase[1];
							else if(mbase[1].isOne()) mpow = &mbase[0];
						}
						if(mpow) {
							if(mpow->isMultiplication()) {
								MathStructure *mpow2 = NULL;
								for(size_t i = 0; i < mpow->size(); i++) {
									MathStructure *mchild = mpow->getChild(i + 1);
									if(!mpow2 && mchild->isPower() && mchild->base()->contains(x_var, true) && mchild->exponent()->representsNegative()) {
										mpow2 = mchild;
									} else if(mchild->contains(x_var, true)) {
										mpow2 = NULL;
										break;
									}
								}
								mmul = mpow;
								mpow = mpow2;
							} else if(!mpow->isPower() || !mpow->exponent()->representsNegative() || !mpow->base()->contains(x_var, true)) {
								mpow = NULL;
							}
						}
						if(mpow) {
							const Number *npow = NULL;
							if(mpow->exponent()->isVariable() && mpow->exponent()->variable()->isKnown() && ((KnownVariable*) mpow->exponent()->variable())->get().isNumber()) {
								npow = &((KnownVariable*) mpow->exponent()->variable())->get().number();
							} else if(mpow->exponent()->isNumber()) {
								npow = &mpow->exponent()->number();
							}
							if(npow) {
								MathStructure mexp(mbak[1]);
								calculate_limit_sub(mexp, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, true);
								MathStructure mbase2(*mpow->base());
								if(!npow->isMinusOne()) {
									mbase2 ^= *npow;
									mbase2.last().calculateNegate(eo);
								}
								calculate_limit_sub(mbase2, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, true);
								int cmp = limit_inf_cmp(mexp, mbase2, x_var);
								if(cmp > 0) {
									calculate_limit_sub(mbase2, x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, false);
									if(mbase2.isInfinite(false)) {
										if(mmul) {
											MathStructure mmul2(*mmul);
											for(size_t i = 0; i < mmul->size(); i++) {
												if(mmul->getChild(i + 1) == mpow) {
													mmul2.delChild(i + 1, true);
													break;
												}
											}
											if(mmul2.representsNegative()) {
												if(mbase2.number().isPlusInfinity()) {
													if(mstruct[1].number().isPlusInfinity()) mstruct.clear(true);
													else mstruct.set(nr_plus_inf, true);
												} else {
													if(mstruct[1].number().isMinusInfinity()) mstruct.clear(true);
													else mstruct.set(nr_plus_inf, true);
												}
												break;
											} else if(mmul2.representsPositive()) {
												if(mbase2.number().isMinusInfinity()) {
													if(mstruct[1].number().isPlusInfinity()) mstruct.clear(true);
													else mstruct.set(nr_plus_inf, true);
												} else {
													if(mstruct[1].number().isMinusInfinity()) mstruct.clear(true);
									...
									mstruct.set(CALCULATOR->v_e, true);
									if(mmul) {
										mstruct ^= *mmul;
										for(size_t i = 0; i < mmul->size(); i++) {
											if(mmul->getChild(i + 1) == mpow) {
												mstruct.last().delChild(i + 1, true);
												break;
											}
										}
									}
				...
				if(keep_inf_x && (mstruct[0].isInfinite(false) || mstruct[1].isInfinite(false))) {
					...
					if(mstruct.isInfinite(false)) {
						...
						if(mstruct[0].isInfinite(false)) {
							mstruct[0] = mbak[0];
							calculate_limit_sub(mstruct[0], x_var, nr_limit, eo, approach_direction, NULL, lhop_depth, true);
						}
						if(mstruct[1].isInfinite(false)) {
							...
							if(mstruct[1].number().isMinusInfinity()) {
								mstruct[1].calculateNegate(eo);
								mstruct[0].calculateInverse(eo);
							}
						}
					}
				}
			...
			if(mstruct[0].isNumber() && !mstruct[0].number().isNonZero() && mstruct[1].representsNegative() && mbak[0].contains(x_var, true) > 0) {
				...
				if(b_test && mstruct[1].representsEven()) {
					i_sgn = 1;
					b_test = false;
				} else if(b_test) {
					if(mstruct[0].number().isInterval() && !mstruct[0].number().isNonNegative() && !mstruct[0].number().isNonPositive()) {
						...
						if(mdiff.differentiate(x_var, eo) && !mdiff.containsFunction(CALCULATOR->f_diff, true)) {
		...
		for(size_t i = 0; i < mstruct.size(); i++) {
			if(mstruct[i].isPower() && mstruct[i][1] == x_var && (nr_limit.number().isMinusInfinity() || mstruct[i][0].representsNonNegative())) {
				...
				if(eo.approximation == APPROXIMATION_EXACT) {
					EvaluationOptions eo2 = eo;
					eo2.approximation = APPROXIMATION_APPROXIMATE;
					mstruct[i][0].calculateFunctions(eo2);
					mstruct[i][0].calculatesub(eo2, eo2, true);
				}
			...
			} else if(eo.approximation == APPROXIMATION_EXACT && mstruct[i].isPower() && mstruct[i][0] == x_var) {
				EvaluationOptions eo2 = eo;
				eo2.approximation = APPROXIMATION_APPROXIMATE;
				mstruct[i][1].calculateFunctions(eo2);
				mstruct[i][1].calculatesub(eo2, eo2, true);
				mstruct[i].childUpdated(2);
				mstruct.childUpdated(i + 1);
			}
		}
	}
		...
		if(mv.isNumber()) {
			...
			for(size_t i = 0; i < mv.size(); i++) {
				if(mv[i].isNumber()) {
					...
					else if(CALCULATOR->usesIntervalArithmetic() && mv[i].number().precision() >= 0) {var_prec = mv[i].number().precision(); break;}
				}
			}
		}		
...
bool contains_undefined(const MathStructure &m) {
	if(m.isPower() && m[0].isNumber() && !m[0].number().isNonZero() && m[1].isNumber() && !m[1].number().isNonNegative()) return true;
	for(size_t i = 0; i < m.size(); i++) {
		if(contains_undefined(m[i])) return true;
	}
	return false;
}
bool find_interval_zeroes(const MathStructure &mstruct, MathStructure &malts, const MathStructure &mvar, const Number &nr_intval, const EvaluationOptions &eo, int depth, const Number &nr_prec) {
	if(CALCULATOR->aborted()) return false;
	MathStructure mtest(mstruct);
	...
	if(COMPARISON_IS_NOT_EQUAL(cmp)) {
		...
		if(cmp == COMPARISON_RESULT_EQUAL || (nr_intval.precision(1) > PRECISION + 15 || (!nr_intval.isNonZero() && nr_intval.uncertainty().isLessThan(nr_prec)))) {
		...
		for(size_t i = 0; i < splits.size(); i++) {
			if(!find_interval_zeroes(mstruct, malts, mvar, splits[i], eo, depth + 1, nr_prec)) return false;
		}
		return true;
	}
	return false;
...
void solve_intervals2(MathStructure &mstruct, vector<KnownVariable*> vars, const EvaluationOptions &eo_pre) {
	if(vars.size() > 0) {
		...
		if(b) {
			...
			if(contains_undefined(msolve) || msolve.countTotalChildren(false) > 1000 || msolve.containsInterval(true, true, false, true)) {
				b = false;
			} else {
				eo.approximation = APPROXIMATION_APPROXIMATE;
				Number nr_prec(1, 1, -(PRECISION + 15));
				nr_prec *= nr_intval.uncertainty();
				b = find_interval_zeroes(msolve, malts, mvar, nr_intval, eo, 0, nr_prec);
				eo.approximation = APPROXIMATION_EXACT_VARIABLES;
			}
...
bool MathStructure::factorize(const EvaluationOptions &eo_pre, bool unfactorize, int term_combination_levels, int max_msecs, bool only_integers, int recursive, struct timeval *endtime_p, const MathStructure &force_factorization, bool complete_square, bool only_sqrfree) {
	...
	switch(type()) {
		case STRUCT_ADDITION: {
			...
			if(term_combination_levels >= -1 && !only_sqrfree) {
				...
				if(SIZE == 2) {
					...
					if(b) {
						...
						if(recursive) {
							factorize(eo, false, term_combination_levels, 0, only_integers, recursive, endtime_p, force_factorization, complete_square, only_sqrfree);
							mmul->factorize(eo, false, term_combination_levels, 0, only_integers, recursive, endtime_p, force_factorization, complete_square, only_sqrfree);
						}
						multiply_nocopy(mmul);
						evalSort(true);
						return true;
				...
				if(!only_integers && !force_factorization.isUndefined() && SIZE >= 2) {
			...
			//complete the square
			if(term_combination_levels != 0 || complete_square) {
				if(only_integers) {
					if(SIZE <= 3 && SIZE > 1) {
						MathStructure *xvar = NULL;
						Number nr2(1, 1);
			...
			if(term_combination_levels != 0 && SIZE > 2) {
				...
				do {
					...
					if(mtest.factorize(eo, false, run_index == 0 ? 0 : -1 - run_index, 0, only_integers, false, endtime_p, force_factorization, complete_square, only_sqrfree)) {
				...
				if(best_index >= 0) {
					...
					if(term_combination_levels >= -1 && (run_index > 0 || recursive)) {
						factorize(eo, false, term_combination_levels, 0, only_integers, true, endtime_p, force_factorization, complete_square, only_sqrfree);
					}
					return true;
				}
			}
		...
		default: {
			...
			if(isComparison()) {
				...
				eo2.assume_denominators_nonzero = false;
				for(size_t i = 0; i < SIZE; i++) {
					if(CHILD(i).factorize(eo2, false, term_combination_levels, 0, only_integers, recursive, endtime_p, force_factorization, complete_square, only_sqrfree)) {
						CHILD_UPDATED(i);
						b = true;
					}
				}
				...
				for(size_t i = 0; i < SIZE; i++) {
					if(CHILD(i).factorize(eo, false, term_combination_levels, 0, only_integers, recursive, endtime_p, force_factorization, complete_square, only_sqrfree)) {
						CHILD_UPDATED(i);
						b = true;
					}
				}
	...
	switch(m_type) {
		...
		case STRUCT_NUMBER: {
			if(o_number.isNegative() || ((parent || po.interval_display != INTERVAL_DISPLAY_SIGNIFICANT_DIGITS) && o_number.isInterval() && o_number.isNonPositive())) {
				if((po.base != 2 || !po.twos_complement) && (!o_number.isMinusInfinity() || (parent && parent->isAddition()))) {
					o_number.negate();
					transform(STRUCT_NEGATE);
					formatsub(po, parent, pindex, true, top_parent);
				}
...
int contains_interval_var(const MathStructure &m, bool structural_only, bool check_variables, bool check_functions, bool ignore_high_precision_interval) {
	if(m.type() == STRUCT_NUMBER) {
		if(m.number().isInterval(false)) {
			...
			if(ignore_high_precision_interval) {
				if(m.number().precision() > PRECISION + 10) return 0;
			}
			return 1;
		}
	}
	if(structural_only) {
		for(size_t i = 0; i < m.size(); i++) {
			if(contains_interval_var(m[i], structural_only, check_variables, check_functions, ignore_high_precision_interval)) return 1;
		}
		if(m.type() == STRUCT_VARIABLE && check_variables && m.variable()->isKnown()) {
			return contains_interval_var(((KnownVariable*) m.variable())->get(), structural_only, check_variables, check_functions, ignore_high_precision_interval);
		} else if(m.type() == STRUCT_FUNCTION && check_functions) {
			if(m.functionValue()) {
				return contains_interval_var(*m.functionValue(), structural_only, check_variables, check_functions, ignore_high_precision_interval);
			}
		}
	} else {
		int ret = 0;
		if(m.type() != STRUCT_FUNCTION) {
			for(size_t i = 0; i < m.size(); i++) {
				int retval = contains_interval_var(m[i], structural_only, check_variables, check_functions, ignore_high_precision_interval);
				if(retval == 1) return 1;
				else if(retval < 0) ret = retval;
			}
		}
...
int MathStructure::containsInterval(bool structural_only, bool check_variables, bool check_functions, bool ignore_high_precision_interval) const {
	...
	if(structural_only) {
		...
		if(m_type == STRUCT_VARIABLE && check_variables && o_variable->isKnown()) {
			return contains_interval_var(((KnownVariable*) o_variable)->get(), structural_only, check_variables, check_functions, ignore_high_precision_interval);
		} else if(m_type == STRUCT_FUNCTION && check_functions) {
			if(function_value) {
	...
	if(mstruct.function() == CALCULATOR->f_ln && mstruct.size() == 1) {
		...
		if(mpow.isOne() && integrate_info(mstruct[0], x_var, madd, mmul, mexp) && mexp.isOne()) {
			if(mfac.isOne()) {
				...
				MathStructure mfacexp(1, 1, 0);
				if(mfac != x_var) mfacexp = mfac[1];
				if(mfacexp.isMinusOne() && madd.isZero()) {
					mstruct.setFunction(CALCULATOR->f_Li);
...
MathStructure *find_abs_x(MathStructure &mstruct, const MathStructure &x_var, const MathStructure *parent = NULL, int level = 0) {
	if(mstruct.isFunction()) {
	...
	for(size_t i = 0; i < mstruct.size(); i++) {
		MathStructure *m = find_abs_x(mstruct[i], x_var, &mstruct, level + 1);
		if(m) return m;
	}
	return NULL;
}
