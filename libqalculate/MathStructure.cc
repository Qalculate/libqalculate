...
int MathStructure::merge_multiplication(MathStructure &mstruct, const EvaluationOptions &eo, MathStructure *mparent, size_t index_this, size_t index_mstruct, bool reversed, bool do_append) {
	...
	if(mstruct.isFunction()) {
		...
		} else if(mstruct.function() == CALCULATOR->f_ln && mstruct.size() == 1 && isZero() && mstruct[0].representsNumber()) {
			if(mstruct[0].representsNonZero() || warn_about_assumed_not_value(mstruct[0], m_zero, eo)) {
				MERGE_APPROX_AND_PREC(mstruct)
				return 2;
			}
		} else if(mstruct.function() == CALCULATOR->f_li && mstruct.size() == 1 && isZero()) {
	...
	switch(m_type) {
		...
		case STRUCT_FUNCTION: {
			...
			} else if(o_function == CALCULATOR->f_ln && SIZE == 1 && mstruct.isZero() && CHILD(0).representsNumber()) {
...
bool integrate_info(const MathStructure &mstruct, const MathStructure &x_var, MathStructure &madd, MathStructure &mmul, MathStructure &mexp, bool radunit = false) {
	if(radunit && mstruct.isMultiplication() && mstruct.size() == 2 && mstruct[1] == CALCULATOR->getRadUnit()) return integrate_info(mstruct[0], x_var, madd, mmul, mexp, false);
	mexp = m_one;
	madd.clear();
	mmul = m_one;
	if(mstruct == x_var) {
		...
		for(size_t i = 0; i < mstruct.size(); i++) {
			if(!b_x && mstruct[i] == x_var) {
				b_x = true;
				i_x = i;
			} else if(!b_x && mstruct[i].isPower() && mstruct[i][0] == x_var) {
				b_x = true;
				i_x = i;
				mexp = mstruct[i][1];
		...
		if(mstruct.size() == 2) {
			...
			if((i_x == 1 && i_rad == 2) || (i_x == 2 && i_rad == 1)) mmul = mstruct[0];
			else if((i_x == 0 && i_rad == 2) || (i_x == 2 && i_rad == 0)) mmul = mstruct[1];
			else mmul = mstruct[2];
		} else {
			mmul = mstruct;
		...
		for(size_t i = 0; i < mstruct.size(); i++) {
			if(mstruct[i] == x_var) {
				if(mexp.isOne()) mmul.addChild(m_one);
				else return false;
			} else if(mstruct[i].isPower() && mstruct[i][0] == x_var) {
				if(mmul.size() == 0) {
					mexp = mstruct[i][1];
				...
				for(size_t i2 = 0; i2 < mstruct[i].size(); i2++) {
					if(!b_x && mstruct[i][i2] == x_var) {
						if(!mexp.isOne()) return false;
						i_x = i2;
						b_x = true;
					} else if(!b_x && mstruct[i][i2].isPower() && mstruct[i][i2][0] == x_var) {
						if(mmul.size() == 0) {
							mexp = mstruct[i][i2][1];
...
bool integrate_function(MathStructure &mstruct, const MathStructure &x_var, const EvaluationOptions &eo, const MathStructure &mpow, const MathStructure &mfac) {
	if(mstruct.function() == CALCULATOR->f_ln && mstruct.size() == 1) {
		...
		if(!mfac.isOne() || !mpow.isNumber() || !mpow.number().isInteger() || !mpow.number().isLessThanOrEqualTo(10) || !mpow.number().isGreaterThanOrEqualTo(-1)) return false;
		MathStructure mexp, mmul, madd;
		if(!integrate_info(mstruct[0], x_var, madd, mmul, mexp) || !mexp.isOne()) return false;
		if(mpow.isOne()) {
			if(madd.isZero()) {
				mstruct.multiply(x_var);
				mstruct.subtract(x_var);
			} else {
			...
			nfac.factorial();
			for(size_t i = 0; i <= n; i++) {
				MathStructure mterm(CALCULATOR->f_ln, &x_var, NULL);
				mterm ^= Number(i);
				mterm *= nfac;
				Number ifac(i);
				ifac.factorial();
				mterm /= ifac;
				MathStructure m1pow(m_minus_one);
				m1pow ^= Number(n - i);
				mterm *= m1pow;
		...
		if(!mfac.isOne() || !mpow.isNumber() || !mpow.number().isInteger() || !mpow.number().isLessThanOrEqualTo(10) || !mpow.number().isGreaterThanOrEqualTo(-2) || mpow.isZero()) return false;
		MathStructure mexp, mmul, madd;
...
bool MathStructure::integrate(const MathStructure &x_var, const EvaluationOptions &eo, bool simplify_first) {
	...
	switch(m_type) {
		case STRUCT_ADDITION: {
			bool b = false;
			for(size_t i = 0; i < SIZE; i++) {
				if(CHILD(i).integrate(x_var, eo)) b = true;
				CHILD_UPDATED(i);
			}
			if(!b) return false;
			break;
		...
		case STRUCT_POWER: {
			if(CHILD(1).isNumber() || CHILD(1).containsRepresentativeOf(x_var, true, true) == 0) {
				...
				MathStructure madd, mmul, mexp;
				if(integrate_info(CHILD(0), x_var, madd, mmul, mexp)) {
					if(mexp.isOne()) {
						if(madd.isZero() && mmul.isOne()) {
							if(b_minusone) {
					...
					if(integrate_function(CHILD(0), x_var, eo, CHILD(1), m_one)) {
						SET_CHILD_MAP(0)
						return true;
					}
				}
				if(CHILD(0).isAddition()) {
					...
					if(mtest[0].factorize(eo, false, 0, 0, false, false)) {
						mmul = m_one;
						if(mtest[0].isMultiplication() && mtest[0].size() >= 2 && mtest[0][0].isNumber()) {
							mmul = mtest[0][0];
							mmul.inverse();
							mtest[0].delChild(1, true);
						}
						MathStructure mmul2(1, 1, 0);
						if(mtest[0].isMultiplication() && mtest[0].size() >= 2 && !mtest[0].isAddition()) {
							mmul2 = mtest[0][0];
						...
						if(mtest.integrate(x_var, eo, false)) {
							set(mtest);
				...
				if(!CHILD(0).isNumber()) {
					ComparisonResult cr = CHILD(0).compareApproximately(m_one, eo);
		...
		case STRUCT_FUNCTION: {
			if(!integrate_function(*this, x_var, eo, m_one, m_one)) CANNOT_INTEGRATE
			break;
		}
		case STRUCT_MULTIPLICATION: {
			MathStructure mstruct;
			...
			if(b) {
				...
				if(CHILD(1).isPower() && CHILD(1)[1].isNumber() && CHILD(1)[1].number().isNegative()) {
					if(CHILD(0) == x_var) {
						MathStructure madd, mmul, mexp;
						if(integrate_info(CHILD(1)[0], x_var, madd, mmul, mexp) && mexp.isOne()) {
							mexp = CHILD(1)[1];
							SET_CHILD_MAP(1)
							SET_CHILD_MAP(0)
					...
					} else if(CHILD(0).isPower() && CHILD(0)[0] == x_var && CHILD(0)[1] == nr_minus_one) {
						MathStructure madd, mmul, mexp;
						if(integrate_info(CHILD(1)[0], x_var, madd, mmul, mexp) && mexp.isOne() && !madd.isZero()) {
							mexp = CHILD(1)[1];
							if(mexp.number().isMinusOne()) {
								SET_CHILD_MAP(1)
								SET_CHILD_MAP(0)
					...
					} else if(CHILD(0).isPower() && CHILD(0)[0] == x_var && CHILD(0)[1] == -2) {
						MathStructure madd, mmul, mexp;
						if(integrate_info(CHILD(1)[0], x_var, madd, mmul, mexp) && mexp.isOne() && !madd.isZero()) {
							mexp = CHILD(1)[1];
							if(mexp.number().isMinusOne()) {
					...
					if(CHILD(1)[0].isAddition()) {
						...
						if(mtest[1][0].factorize(eo, false, 0, 0, false, false)) {
							MathStructure mmul(m_one);
							if(mtest[1][0].isMultiplication() && mtest[1][0].size() >= 2 && mtest[1][0][0].isNumber()) {
								mmul = mtest[1][0][0];
								mmul.inverse();
								mtest[1][0].delChild(1, true);
							}
							MathStructure mmul2(1, 1, 0);
							if(mtest[1][0].isMultiplication() && mtest[1][0].size() >= 2 && !mtest[1][0].isAddition()) {
								mmul2 = mtest[1][0][0];
								mmul2.calculateInverse(eo2);
								mtest[1][0].delChild(1, true);
							}
					...
					int best_type = 10;
					for(size_t i = 0; i < SIZE; i++) {
						if(best_type > 1 && (CHILD(i) == x_var || (CHILD(i).isPower() && !CHILD(i)[0].containsType(STRUCT_FUNCTION) && CHILD(i)[1].containsRepresentativeOf(x_var, true, true) == 0 && CHILD(i)[1].representsNonNegative()))) {
							u_index = i;
							best_type = 1;
						} else if(CHILD(i).isFunction() && CHILD(i).function() == CALCULATOR->f_ln) {
							if(best_type == 2 || best_type == 3) {u_index = SIZE; break;}
							if(best_type != 0) {u_index = i; best_type = 0;}
						} else if(CHILD(i).isFunction() && (CHILD(i).function() == CALCULATOR->f_sin || CHILD(i).function() == CALCULATOR->f_sinh)) {
							if(best_type == 0 || best_type == 2 || best_type == 3) {u_index = SIZE; break;}
							if(best_type > 2) {u_index = i; best_type = 2;}
						} else if(CHILD(i).isFunction() && (CHILD(i).function() == CALCULATOR->f_cos || CHILD(i).function() == CALCULATOR->f_cosh)) {
							if(best_type == 0 || best_type == 2 || best_type == 3) {u_index = SIZE; break;}
							if(best_type > 3) {u_index = i; best_type = 3;}
						} else if(CHILD(i).isPower() && CHILD(i)[0].containsRepresentativeOf(x_var, true, true) != 0 && !CHILD(i)[1].representsNonNegative()) {
							u_index = SIZE;
							break;
						}
					}
			...
			if(SIZE >= 2) {
				for(size_t i = 0; i < SIZE; i++) {
					if(CHILD(i).isFunction()) {
						...
						if(integrate_function(CHILD(i), x_var, eo, m_one, mmul)) {
							SET_CHILD_MAP(i)
							return true;
						}
						...
						MathStructure mmul(*this);
						mmul.delChild(i + 1, true);
						if(integrate_function(CHILD(i)[0], x_var, eo, CHILD(i)[1], mmul)) {
							SET_CHILD_MAP(i)
							SET_CHILD_MAP(0)
							return true;
						}
